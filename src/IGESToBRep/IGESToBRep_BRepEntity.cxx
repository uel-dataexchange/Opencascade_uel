	...
	//AddWarning ( start, "Shell is not closed" ); //:39
	  ...
	  // 複合面の配置対応
		if (start->HasTransf()) {
			...
			(GetEpsilon(),start->CompoundLocation(), trsf, GetUnitFactor())) { 
				if(!S.IsNull())
				{
					TopoDS_Shell retSh =  TopoDS::Shell( MoveShell(S, start, trsf));
					if(!retSh.IsNull())
					{
						S = retSh;
					}
  ...
  (const Handle(IGESSolid_ManifoldSolid)& start)
{
  ...
  if (!HasShapeResult(start)) {
	...
	Standard_Boolean hasTransf = Standard_False;
	gp_Trsf trsf;
	// 立体の配置対応
	if (start->HasTransf()) {
		SetEpsilon(1.E-04);
	...
	if(hasTransf)
		Shell = TopoDS::Shell( MoveShell(Shell, shell, trsf) );
	B.Add(S,Shell);
      }
...
//	  Standard_Boolean orientation = start->VoidOrientationFlag(ishell);
	  ...
	  if (!aSh.IsNull()) {
	    if (aSh.ShapeType() == TopAbs_SHELL) {
	      ...
	      if (!isoriented) Shell.Reverse();
		  if(hasTransf)
				Shell = TopoDS::Shell( MoveShell(Shell, voidshell, trsf) );
	      B.Add(S,Shell);
	    }
	  }
	  else {
...
// Move shell to trsf
TopoDS_Shape IGESToBRep_BRepEntity::MoveShell(TopoDS_Shape shpShell, const Handle_IGESSolid_Shell &shell, gp_Trsf trsf)
{
	if(!shpShell.IsNull())
	{
		BRepBuilderAPI_Transform transfor(trsf);
		transfor.Perform(shpShell, Standard_True);
		TopoDS_Shape ret = transfor.Shape();
		if(!ret.IsNull())
		{
			Standard_Integer nbfaces = shell->NbFaces();
			for(Standard_Integer i = 1; i<=nbfaces; i++)
			{
				Handle(IGESSolid_Face) face = shell->Face(i);
				TopoDS_Shape shFace = GetShapeResult(face);
				if(!shFace.IsNull())
				{
					TopoDS_Shape mFc = transfor.ModifiedShape(shFace);
					SetShapeResult(face, mFc);
				}
			}
			SetShapeResult(shell, ret);
			return ret;
		}
	}
