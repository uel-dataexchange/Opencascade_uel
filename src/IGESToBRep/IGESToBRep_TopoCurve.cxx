  ...
  if (GetModeTransfer() && start->HasTransf()) {
	...
	(GetEpsilon(),start->CompoundLocation(),T,GetUnitFactor())) { 
      ...
      //V1.Move(L);
		if(!V1.IsNull())
		{
			BRepBuilderAPI_Transform transfor(T);
			transfor.Perform(V1, Standard_True);
			V1 = TopoDS::Vertex(transfor.Shape());
		}
    }
  }
						       ...
						       const Standard_Boolean isCurveOnSurf)
{
  ...
  if (start->HasTransf()) {
	...
	(GetEpsilon(),start->CompoundLocation(),T)) {
      ...
      //mywire.Move(L);
		...
		{
			BRepBuilderAPI_Transform transfor(T);
			transfor.Perform(mywire, Standard_True);
			mywire = TopoDS::Wire(transfor.Shape());
		}
    }
    else {
  ...
  Standard_Integer opt_con = Interface_Static::IVal("read.iges.opt_con");
  if (opt_con == 1)
  {
	  // 境界線をC1連続に分割する
	  ShapeUpgrade_ShapeDivideContinuity condiv(mywire);
	  condiv.SetPrecision(0.01);
	  condiv.SetMinTolerance(0.01);
	  condiv.SetBoundaryCriterion(GeomAbs_C1);
	  if (condiv.Perform())
	  {
		  mywire = TopoDS::Wire(condiv.Result());
	  }
  }
...
// le souhaite, on approxime
  TheCurves.Clear();
  Standard_Integer optk = Interface_Static::IVal("read.iges.opt_k");
  if(start->IsKind(STANDARD_TYPE(IGESGeom_CopiousData)) && optk > 0) {
	  ...
	  if(optk == 1) {
		  // 有意点列データを線分に変換
		  for(Standard_Integer idx = 1; idx <=BSplineC->NbPoles()-1; idx++) {
			  BRepLib_MakeEdge mkEdg(BSplineC->Pole(idx), BSplineC->Pole(idx+1));
			  if(mkEdg.IsDone()) {
				  builder.Add(cmp, mkEdg.Edge());
			  }
		  ...
		  // 有意点列データを点群に変換
		  for(Standard_Integer idx = 1; idx <=BSplineC->NbPoles(); idx++) {
			TopoDS_Vertex vtx;
			builder.MakeVertex(vtx, BSplineC->Pole(idx), Precision::Confusion());
			builder.Add(cmp, vtx);
		  }
	  }
  ...
  else if ((C->IsKind(STANDARD_TYPE(Geom_BSplineCurve)))&& GetModeApprox()) {
	Handle(Geom_BSplineCurve) BSplineC = Handle(Geom_BSplineCurve)::DownCast(C);
...
//  B.UpdateVertex (V2, epsgeom);
  ...
  if (start->HasTransf() && bIsNeedTransf) {
    ...
    if (IGESData_ToolLocation::ConvertLocation
      ...
      TopLoc_Location L(T);
	  BRepBuilderAPI_Transform transfor(T);
	  transfor.Perform(myshape, Standard_True);
	  myshape = transfor.Shape();
      //myshape.Move(L);
    }
    else {
      Message_Msg Msg1035("IGES_1035");
