  ...
  if (st->HasTransf()) {
	...
	(GetEpsilon(),st->CompoundLocation(),trsf,GetUnitFactor())) { 
      ...
      //res.Move(locFace);
		if(!res.IsNull())
		{
			BRepBuilderAPI_Transform transfor(trsf);
			transfor.Perform(res, Standard_True);
			res = transfor.Shape();
		}
    }
    else {
...
{ // Declaration of messages// 
  ...
  Standard_Integer opt_ar = Interface_Static::IVal("read.iges.opt_ar");
  ...
  if (!srgen.IsNull()) {
	...
	bscrvAxis = gp_Ax1(gen1, gp_Dir( gp_Vec (gen2, gen1)));
	length = gen2.Distance(gen1);
  }
  ...
  try {
    ...
    if (extractCurve3d(generatrix, aBasisCurve)) {
		...
		Handle(Geom_Curve) bCrv;
		try
		{
			if(aBasisCurve->IsKind(STANDARD_TYPE(Geom_TrimmedCurve)))
			{

				bCrv = Handle(Geom_TrimmedCurve)::DownCast(aBasisCurve)->BasisCurve();
			}
			else
			{
				bCrv = aBasisCurve;
			}
			if(bCrv->IsKind(STANDARD_TYPE(Geom_Circle)))
			{

				//const Handle(Geom_Circle) conic = Handle(Geom_Circle)::DownCast(bCrv);
				//const gp_Pnt pc = conic->Location();
				//gp_Ax1 a1 = gp_Ax1(pt1, gp_Dir(gp_Vec(pt1, pt2)));
				//const gp_Lin rl (a1);
				//if (rl.Distance(pc) < Precision::Confusion()) { //pc lies on A2
				//	// transfor as a sphere
				//	const gp_Dir dirline = a1.Direction();
				//	const gp_Dir norm = conic->Axis().Direction();
				//	if( dirline.IsNormal(norm,0.001) ) { //A2 lies on plane of circle
				//		//change parametrization for trimming
				//		// change to sphere
				//		gp_Ax3 ax3(pc, dirline);
				//		aResultSurf = new Geom_SphericalSurface(ax3, conic->Radius());
				//	}
				//}

				aBasisCurve = GeomConvert::CurveToBSplineCurve(aBasisCurve);
			}
		}
		...
		{
			aResultSurf = new Geom_SurfaceOfRevolution(aBasisCurve, revolAxis);
			if (opt_ar == 1) {
				// 回転面を自由曲面に近似する
				aResultSurf = GeomConvert::SurfaceToBSplineSurface(aResultSurf);
			}
			
			
		}

		//Handle(Geom_Line) axisLine = new Geom_Line(revolAxis);
...
{ // Declaration of messages// 
  ...
  // 基底面の表示ステータス設定
  igesSurface->InitStatus(st->BlankStatus(), igesSurface->SubordinateStatus(), igesSurface->UseFlag(), igesSurface->HierarchyStatus());
  
  Standard_Integer opt_ap = Interface_Static::IVal("read.iges.opt_ap");
  if(opt_ap != 1) {
	  // 基底面のレイヤ情報を使用する
	  st->InitLevel(0, igesSurface->Level());
  }
  else {
	  // トリム面のレイヤ情報を使用する
	  igesSurface->InitLevel(0, st->Level());
  }
  ...
  if (st->HasTransf()) {
	  ...
	  if (IGESData_ToolLocation::ConvertLocation
		  ...
		  {
			  BRepBuilderAPI_Transform transfor(trsf);
			  transfor.Perform(face, Standard_True);
			  face = TopoDS::Face(transfor.Shape());
		  }
	  }
	  else {
		  Message_Msg msg1035("IGES_1035");
...
{ // Declaration of messages// 
  ...
  if (first) {
	...
	if(abs(st->SymbolSize()) >	Precision::Confusion()) 
	{
		gp_Ax3 pos = geomPln->Position();
		pos.SetLocation(st->SymbolAttach());
		geomPln->SetPosition(pos);
	}
