...
{
  int i,i0,j,k; char param[80]; char unpar, lastvar;
  if (*Pstat == 0) reste  = 0;
  if (*Pstat != 2) numcar = 0;
  if (*Pstat < 3)  nbcarH = 0;
  else if(*Pstat == 9){
	  for(k=0; (unpar = ligne[numcar+k]) != '\0'; k ++) {
		  nbcarH = k;
		  if (unpar == '\0' || unpar == c_fin || unpar ==c_separ) {
			  break;
		  }
	  }
	  numcar = nbcarH+1;
...
/*   blanc : leading (facile) ou trailing (chercher la suite), sinon mauvais */
	...
	if (lastvar != '\0' && lastvar != c_fin && lastvar !=c_separ) {
		*Pstat = 9;
	}
	else {
