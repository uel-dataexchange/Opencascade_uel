...
{
  char *newval, *oldval, paramHChar[64], curval; int i,long0, nbchr, chrFlg;
  paramHChar[0] = '\0';
  chrFlg = 0;
  nbchr = 0;
  if (longval <= 0) return;
...
/*  newval = (char*) malloc(long0+longval+1);  */
  ...
  for (i = 0; i < longval; i ++) {
	  curval=parval[i];
	  newval[i+long0] = curval;
	  if(chrFlg)
	  {
		paramHChar[nbchr] = curval;
		nbchr++;
	  }
	  if (curparam->typarg == ArgChar) {
		  continue;
	  }
	  if (curval > 47 && curval < 58) 
	  {
		if (curparam->typarg== ArgInt) 
			continue;
		if (curparam->typarg == ArgVide) 
			curparam->typarg = ArgInt;
		else if (curparam->typarg == ArgExp) 
			curparam->typarg = ArgRexp;
	  }
	  else if (curval == '+' || curval == '-') 
	  {
		if (curparam->typarg == ArgVide) 
			curparam->typarg = ArgSign;
        else if (curparam->typarg != ArgExp && curparam->typarg != ArgMexp) 
			curparam->typarg = ArgQuid;
	  }
      else if (curval == '.') 
	  {
		if (curparam->typarg == ArgVide) 
			curparam->typarg = ArgReal;
		else if (curparam->typarg == ArgInt || curparam->typarg == ArgSign) 
			curparam->typarg = ArgReal;
		else 
			curparam->typarg = ArgQuid;
	  }

	  else if (curval == 'E' || curval == 'e' || curval == 'D' || curval == 'd') 
	  {
		if (curparam->typarg == ArgReal) 
			curparam->typarg = ArgExp;
		else if (curparam->typarg == ArgInt || curparam->typarg == ArgSign) 
			curparam->typarg = ArgMexp;
		else 
			curparam->typarg = ArgQuid;
	  }
      else if (curval == 'H') 
