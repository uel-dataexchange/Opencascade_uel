...
IMPLEMENT_STANDARD_RTTIEXT_1(STEPControl_ActorWrite,Transfer_ActorOfFinderProcess)
          ...
          // modified by PTV 16.09.2002 OCC725
            ...
            if ( aNbVrtx ) {
              ...
              for ( anExp.ReInit() ; anExp.More(); anExp.Next() ) {
				...
				// 2012/01/13C³
				Handle(TransferBRep_ShapeMapper) vtxMapper = TransferBRep::ShapeMapper(FP, aVertex);
				Handle(Transfer_Binder) vtxbind = FP->Find(vtxMapper);
				if (vtxbind.IsNull())
				{
					vtxbind = TransientResult(aGSS->Value(curNb).Value());
					FP->Bind(vtxMapper, vtxbind);
				}
				else
					vtxbind->AddResult(TransientResult(aGSS->Value(curNb).Value()));
