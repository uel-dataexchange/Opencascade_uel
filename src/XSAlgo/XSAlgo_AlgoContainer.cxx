                                                 ...
                                                 Handle(Standard_Transient)& info) const
{
  ...
  // fix small
  Standard_Integer ifix = 0;
  Standard_Character outStr[16];
  Standard_CString cval;
  memset(outStr, NULL, sizeof(CHAR) * 16);
  ifix = Interface_Static::IVal("read.fixsmall");
  Standard_CString cseq = Interface_Static::CVal(pseq);
  _itoa(ifix, outStr, 10);
  cval = outStr;
  Handle(TCollection_HAsciiString) str_res;
  str_res = new TCollection_HAsciiString(cseq);
  str_res->AssignCat(".FixShape.read.fixsmall");
  rsc->SetResource(str_res->String().ToCString(), cval);
