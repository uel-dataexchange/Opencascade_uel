                                  ...
                                  Handle(Interface_Check)& ach)
{
  ...
  // CATIAまたはThinkDesignから出力したIGESの場合、強制にトリム面のレイヤ番号を使うように設定する
  if (!theSystemId.IsNull()) {
	  Handle(TCollection_HAsciiString) lwrSys = new  TCollection_HAsciiString(strlwr((char*)theSystemId->ToCString()));
	  if (lwrSys->Search("catia") > 0 || lwrSys->Search("think") > 0) {
		  Interface_Static::SetIVal("read.iges.opt_ap", 1);
	  }
