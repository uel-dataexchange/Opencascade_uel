...
// Decoupage : 4 bits chacun (BlankStatus tout a droite, etc)
    ...
    void IGESData_IGESEntity::LineWeightNumber (Standard_Integer weightNum) 
	{
    ...
    gp_GTrsf IGESData_IGESEntity::Location () const
{
  ...
  if (trsf.IsNull()) return gp_GTrsf();
  gp_GTrsf loca = trsf->Value();
  if (trsf->HasTransf()) {
	  gp_GTrsf locp = trsf->Location();
    ...
    Standard_Boolean IGESData_IGESEntity::HasName () const
{
  ...
  return (NbTypedProperties(STANDARD_TYPE(IGESData_NameEntity)) == 1 && !NameValue().IsNull());
}
       ...
       const Standard_Integer gradw)
{
  if (theLWeightNum == 0) theLWeightVal = defw;
	...
	void IGESData_IGESEntity::SetDirNumber(Standard_Integer &dirNum)
	{
		if(dirNum > 0)
		{
			theDirNumber = dirNum;
	...
	Standard_Integer IGESData_IGESEntity::DirNumber()
	{
		return theDirNumber;
	}
