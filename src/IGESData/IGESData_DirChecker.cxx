  ...
  (const Handle(IGESData_IGESEntity)& ent) const 
{
  ...
  /*if(type == 308)
  {
	  Handle(IGESBasic_SubfigureDef) subfig = Handle(IGESBasic_SubfigureDef)::DownCast(ent);
	  for(Standard_Integer idx = 1; idx<=subfig->NbEntities(); idx++)
	  {
		Handle(IGESData_IGESEntity) subenty = subfig->AssociatedEntity(idx);
		if(!subenty.IsNull())
		{
			stb = subenty->BlankStatus();
			sts = subenty->SubordinateStatus();
			stu = subenty->UseFlag();
			sth = subenty->HierarchyStatus();
			if(sts != 0)
			{
				sts = 0;
				subenty->InitStatus(stb,sts,stu,sth);
				done = Standard_True;
			}
		}
	  }
