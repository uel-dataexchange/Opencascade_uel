...
//    if (trim1 > 2.1*PI || trim2 > 2.1*PI) fact = PI / 180.;
  ...
  if (FoundParam1 && FoundParam2) {
	...
	if(MasterRep == 1 && theCurve->IsClosed()) {
		if(SC->SenseAgreement() && trim2 == 0) {
			trim2 = cl;
		}
		else if(!SC->SenseAgreement() && trim1 == 0) {
			trim1 = cl;
		}
	}
...
//:o6    trim1 = trim1 * fact;
//:o6    trim2 = trim2 * fact;
	if(theCurve->IsKind(STANDARD_TYPE(Geom_BSplineCurve)) && MasterRep == 1) {
		Handle(Geom_BSplineCurve) bspCrv = Handle(Geom_BSplineCurve)::DownCast(theCurve);
		if(trim1 < trim2) {
			bspCrv->Segment(trim1, trim2);
		}
		else {
			bspCrv->Segment(trim2, trim1);
		}
		theCurve = bspCrv;
	}
