	 ...
	 const Handle(StepShape_OrientedClosedShell)& ent) const
{
	...
	Standard_Boolean stat3 = data->ReadEntity(num, 3,"closed_shell_element", ach, STANDARD_TYPE(StepShape_ClosedShell), aClosedShellElement);
	if(!stat3 && aClosedShellElement.IsNull())
	{
		aClosedShellElement = new StepShape_ClosedShell();
	}
