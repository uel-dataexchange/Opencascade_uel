...
IMPLEMENT_STANDARD_RTTIEXT_1(XCAFDoc_DimTolTool, TDF_Attribute)
...
void XCAFDoc_DimTolTool::GetDimTolLabels(TDF_LabelSequence1& theLabels) const
{
  theLabels.Clear();
...
void XCAFDoc_DimTolTool::GetDimensionLabels(TDF_LabelSequence1& theLabels) const
{
  theLabels.Clear();
  TDF_ChildIterator aChildIterator( Label() ); 
  for (; aChildIterator.More(); aChildIterator.Next()) {
...
void XCAFDoc_DimTolTool::GetGeomToleranceLabels(TDF_LabelSequence1& theLabels) const
{
  theLabels.Clear();
  TDF_ChildIterator aChildIterator( Label() ); 
  for (; aChildIterator.More(); aChildIterator.Next()) {
...
void XCAFDoc_DimTolTool::SetDimension(const TDF_Label& theFirstL,
                                   ...
                                   const TDF_Label& theDimTolL) const
{
  TDF_LabelSequence1 aFirstLS, aSecondLS;
  if(!theFirstL.IsNull())
...
void XCAFDoc_DimTolTool::SetDimension(const TDF_LabelSequence1& theFirstL,
                                   const TDF_LabelSequence1& theSecondL,
                                   const TDF_Label& theDimTolL) const
{
...
void XCAFDoc_DimTolTool::SetGeomTolerance(const TDF_LabelSequence1& theL,
                                          const TDF_Label& theGeomTolL) const
{
  //  // set reference
                                                      ...
                                                      TDF_LabelSequence1& theShapeLSecond) const
{
  ...
  /*if (!theL.FindAttribute(XCAFDoc::DimTolRefGUID(), aNode) || !aNode->HasFather() || aNode->Father()->Label().IsNull()) {
	  if (!theL.FindAttribute(XCAFDoc::DatumRefGUID(), aNode) || !aNode->HasFather() || aNode->Father()->Label().IsNull()) {*/
      ...
      if( theL.FindAttribute(XCAFDoc::GeomToleranceRefGUID(),aGNode) && aGNode->NbFathers() > 0 ) {
        ...
        for (Standard_Integer i = 1; i <= aGNode->NbFathers(); i++)
        {
			Handle(XCAFDoc_GraphNode) fNode = aGNode->GetFather(i);
			TDF_Label l = fNode->Label();
          theShapeLFirst.Append(aGNode->GetFather(i)->Label());
        }
        return Standard_True;
      }
...
void XCAFDoc_DimTolTool::GetDatumLabels(TDF_LabelSequence1& theLabels) const
{
  theLabels.Clear();
  TDF_ChildIterator aChildIterator( Label() ); 
  for (; aChildIterator.More(); aChildIterator.Next()) {
...
void XCAFDoc_DimTolTool::SetDatum(const TDF_LabelSequence1& theL,
