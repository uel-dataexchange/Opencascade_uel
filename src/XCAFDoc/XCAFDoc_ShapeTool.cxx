...
IMPLEMENT_STANDARD_RTTIEXT_1(XCAFDoc_ShapeTool, TDF_Attribute)
...
void XCAFDoc_ShapeTool::GetShapes(TDF_LabelSequence1& Labels) const
{
  Labels.Clear();
...
void XCAFDoc_ShapeTool::GetFreeShapes (TDF_LabelSequence1& FreeLabels) const
{
  FreeLabels.Clear();
...
Standard_Boolean XCAFDoc_ShapeTool::GetComponents (const TDF_Label& L, TDF_LabelSequence1& Labels,
						   const Standard_Boolean getsubchilds) 
{
...
void XCAFDoc_ShapeTool::UpdateAssemblies()
{
  ...
  TDF_LabelSequence1 aRootLabels;
  GetFreeShapes(aRootLabels);

  // Iterate over the free shapes
  for ( TDF_LabelSequence1::Iterator anIt(aRootLabels); anIt.More(); anIt.Next() )
  {
...
Standard_Boolean XCAFDoc_ShapeTool::SetSHUO (const TDF_LabelSequence1& labels,
...
Standard_Boolean XCAFDoc_ShapeTool::FindComponent (const TopoDS_Shape& theShape,
                                                  TDF_LabelSequence1& theLabels) const
{
  theLabels.Clear();
...
Standard_Boolean XCAFDoc_ShapeTool::FindSHUO (const TDF_LabelSequence1& theLabels,
                                                    ...
                                                    TopoDS_Shape&    theUpdatedShape) const
{
  ...
  TDF_LabelSequence1 aComponentLabs;
  ...
  //
  for ( TDF_LabelSequence1::Iterator aCompIt(aComponentLabs); aCompIt.More(); aCompIt.Next() )
  {
