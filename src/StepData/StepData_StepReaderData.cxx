...
#include <stdio.h>
IMPLEMENT_STANDARD_RTTIEXT_1(StepData_StepReaderData, Interface_FileReaderData)
   ...
   Standard_Integer& num, Handle(Interface_Check)& ach) const
{
  ...
  if ((n!=0) && (!stricmp( RecordType(n).ToCString(), name )) )
...
//  Pas dans l ordre alphabetique : boucler
  Handle(TCollection_HAsciiString) errmess = new String("Parameter n0.%d (%s) not a LIST");
  sprintf (txtmes,errmess->ToCString(),num0,name);
  for (n = num0; n > 0; n = NextForComplex(n)) {
    if (!stricmp( RecordType(n).ToCString(), name))  {
      num = n;
   ...
   Handle(Interface_Check)& ach) const
{
  ...
  if ((n!=0) && (!stricmp( RecordType(n).ToCString(), theName) ||
                 !stricmp(RecordType(n).ToCString(), theShortName)) )
  {
    num = n;
  ...
  for (n = num0; n > 0; n = NextForComplex(n))
  {
    if (!stricmp(RecordType(n).ToCString(),theName) ||
        !stricmp(RecordType(n).ToCString(), theShortName))
    {
      num = n;
      errmess = new String("Complex Record n0.%d, member type %s not in alphabetic order");
