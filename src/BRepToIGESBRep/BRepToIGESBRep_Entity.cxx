...
Handle(IGESData_IGESEntity) BRepToIGESBRep_Entity ::TransferFace(const TopoDS_Face& start)
{
  ...
  TopoDS_Wire oWire = ShapeAlgo::AlgoContainer()->OuterWire(start); 
  if(oWire.IsNull())
  {
	  // ‹«ŠEü‚È‚¢–Ê‚Í–³ŒÀ•½–Ê‚Æ‚µ‚Ä•ÏŠ·‚·‚é
	  return TransferPlanFace(start);
  }
...
// –³ŒÀ•½–Ê‚Ì•ÏŠ·
Handle(IGESData_IGESEntity) BRepToIGESBRep_Entity ::TransferPlanFace(const TopoDS_Face& start)
{
  // –³ŒÀ•½–Ê‚Ì‘Î‰ž
  Handle(IGESData_IGESEntity) resultSrf;
  /*TopoDS_Wire oWire = ShapeAlgo::AlgoContainer()->OuterWire(start); 
  if(oWire.IsNull())
  {*/
	Handle(Geom_Surface) srf = BRep_Tool::Surface(start);
	if(!srf.IsNull() && srf->IsKind(STANDARD_TYPE(Geom_Plane)))
	{
		GeomToIGES_GeomSurface GS;
		Handle(Geom_Plane) plan = Handle(Geom_Plane)::DownCast(srf);
		if(!plan.IsNull())
		{
			GS.SetBRepMode(Standard_False);
			GS.SetModel(GetModel());
			resultSrf = GS.TransferSurface(plan, 0, 0, 0, 0);
			if(!resultSrf.IsNull())
			{
				SetShapeResult(start, resultSrf);
			}
		}
	}
  /*}*/
  ...
  for (Ex.Init(start,TopAbs_FACE); Ex.More(); Ex.Next()) {
    ...
    if (F.IsNull()) {
      AddWarning(start," a Face is a null entity");
		...
		Handle(IGESData_IGESEntity) rtEnt = TransferFace(F);
		if(!rtEnt.IsNull())
		{
			IFace = Handle(IGESSolid_Face)::DownCast(rtEnt);
		}
      if (!IFace.IsNull()) {
	Seq->Append(IFace);
