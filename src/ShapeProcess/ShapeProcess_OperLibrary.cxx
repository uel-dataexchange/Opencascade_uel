...
static Standard_Boolean fixfacesize (const Handle(ShapeProcess_Context)& context)
{
  ...
  ctx->GetReal("Tolerance3d", aTol);
  Standard_Real aMaxTol = -1.0;
  aMaxTol = ctx->RealVal("read.maxtol", -1.0);
  if (aMaxTol > aTol) {
	  aTol = aMaxTol;
  }
  else {
  ...
  FSC.SetMaxTolerance(aTol);
  FSC.SetMinTolerance(aTol);
  //FSC.Perform();
  //TopoDS_Shape newsh = FSC.Shape();

  TopoDS_Shape newsh = FSC.RemoveSmallFaces();
  if (!newsh.IsNull() && newsh != ctx->Result()) {
...
static Standard_Boolean fixshape (const Handle(ShapeProcess_Context)& context)
{
  ...
  Standard_Integer iDrp = ctx->IntegerVal("read.fixsmall", 0);
  if (iDrp == 1) {
	  fixwgaps(ctx);
