...
//=======================================================================
//function : STEPConstruct_DefProps
//purpose  : 
//=======================================================================
STEPConstruct_DefProps::STEPConstruct_DefProps () 
{
}
...
STEPConstruct_DefProps::STEPConstruct_DefProps (const Handle(XSControl_WorkSession) &WS)
...
Standard_Boolean STEPConstruct_DefProps::Init (const Handle(XSControl_WorkSession) &WS)
{
  return SetWS ( WS );
}
...
Standard_Boolean STEPConstruct_DefProps::FindTarget (const TopoDS_Shape &Shape,
...
Standard_Boolean STEPConstruct_DefProps::AddProp (const StepRepr_CharacterizedDefinition &target,
...
Standard_Boolean STEPConstruct_DefProps::AddProp (const TopoDS_Shape &Shape,
							 const Handle(StepRepr_RepresentationItem) &Prop,
							 const Standard_CString Descr,
							 const Standard_Boolean instance)
{
...
Standard_Boolean STEPConstruct_DefProps::AddVolume (const TopoDS_Shape &Shape, 
...
Standard_Boolean STEPConstruct_DefProps::AddArea (const TopoDS_Shape &Shape, 
...
Standard_Boolean STEPConstruct_DefProps::AddCentroid (const TopoDS_Shape &Shape, 
							     const gp_Pnt &Pnt,
							     const Standard_Boolean instance)
{
...
Standard_Boolean STEPConstruct_DefProps::LoadProps (TColStd_SequenceOfTransient &seq) const
{
  // parse on PropertyDefinitionRepresentations
  Standard_Integer nb = Model()->NbEntities();
  Handle(Standard_Type) tPDR = STANDARD_TYPE(StepRepr_GeneralPropertyAssociation);
  for (Standard_Integer i = 1; i <= nb; i ++) {
    Handle(Standard_Transient) enti = Model()->Value(i);
    if ( ! enti->IsKind (tPDR) ) continue;
    
	Handle(StepRepr_GeneralPropertyAssociation) GPA =
		Handle(StepRepr_GeneralPropertyAssociation)::DownCast(enti);
    
    // check that PDR is for validation props
	Handle(StepRepr_PropertyDefinition) PD = GPA->ItemWithProperty().PropertyDefinition();
	if (PD->Name())
    if ( PD.IsNull() || PD->Name().IsNull() 
		|| (PD->Name()->Search("validation") > 0 && PD->Name()->Search("property") > 0))
      continue;

	Interface_EntityIterator subs1 = Graph().Sharings(PD);
	for (subs1.Start(); subs1.More(); subs1.Next()) {
		enti = subs1.Value();
...
Standard_Boolean STEPConstruct_DefProps::LoadPropsDef(TColStd_SequenceOfTransient &seq) const
{
	// parse on PropertyDefinitionRepresentations
	Standard_Integer nb = Model()->NbEntities();
	Handle(Standard_Type) tPDR = STANDARD_TYPE(StepRepr_GeneralPropertyAssociation);
...
Handle(StepBasic_ProductDefinition) STEPConstruct_DefProps::GetPropPD (const Handle(StepRepr_PropertyDefinition) &PD) const
{
  StepRepr_CharacterizedDefinition CD = PD->Definition();
    
  // detect target entity of valprop
  Handle(StepBasic_ProductDefinition) ProdDef = CD.ProductDefinition();
  if (!ProdDef.IsNull()) {
	  return ProdDef;
  }
...
Handle(StepRepr_NextAssemblyUsageOccurrence) STEPConstruct_DefProps::GetPropNAUO (const Handle(StepRepr_PropertyDefinition) &PD) const
{
  StepRepr_CharacterizedDefinition CD = PD->Definition();
...
TopoDS_Shape STEPConstruct_DefProps::GetPropShape (const Handle(StepBasic_ProductDefinition) &ProdDef) const
{
  // find target shape
  TopoDS_Shape S;
...
TopoDS_Shape STEPConstruct_DefProps::GetPropShape (const Handle(StepRepr_PropertyDefinition) &PD) const
{
...
Standard_Boolean STEPConstruct_DefProps::GetPropReal (const Handle(StepRepr_RepresentationItem) &item,
							     Standard_Real &Val, Standard_Boolean &isArea) const
{
  // decode volume & area
	if (item->IsKind(STANDARD_TYPE(StepRepr_RealRepresentationItem))) {
		Handle(StepRepr_RealRepresentationItem) RRI = Handle(StepRepr_RealRepresentationItem)::DownCast(item);
		Val = RRI->Value();
		return Standard_True;
	}
...
Standard_Boolean STEPConstruct_DefProps::GetPropInteger(const Handle(StepRepr_RepresentationItem)& item, Standard_Integer& Val) const
{
	// decode volume & area
	if (item->IsKind(STANDARD_TYPE(StepRepr_IntegerRepresentationItem))) {
		Handle(StepRepr_IntegerRepresentationItem) IRI = Handle(StepRepr_IntegerRepresentationItem)::DownCast(item);
		Val = IRI->Value();
		return Standard_True;
	}
...
Standard_Boolean STEPConstruct_DefProps::GetPropString(const Handle(StepRepr_RepresentationItem)& item, Standard_CString& Val) const
{
	// decode volume & area
	if (item->IsKind(STANDARD_TYPE(StepRepr_DescriptiveRepresentationItem))) {
		Handle(StepRepr_DescriptiveRepresentationItem) DRI = Handle(StepRepr_DescriptiveRepresentationItem)::DownCast(item);
		if (!DRI->Description().IsNull())
			Val = DRI->Description()->ToCString();
		else if (!DRI->Name().IsNull())
			Val = DRI->Name()->ToCString();
		else
			return Standard_False;
		return Standard_True;
	}
...
Standard_Boolean STEPConstruct_DefProps::GetPropPnt (const Handle(StepRepr_RepresentationItem) &item,
							    const Handle(StepRepr_RepresentationContext) &Context,
							    gp_Pnt &Pnt) const
{
  // centroid
...
void STEPConstruct_DefProps::SetAssemblyShape (const TopoDS_Shape& shape)
{
  Handle(TransferBRep_ShapeMapper) mapper = TransferBRep::ShapeMapper(FinderProcess(),shape);
