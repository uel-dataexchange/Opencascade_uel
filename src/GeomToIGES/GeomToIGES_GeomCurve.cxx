...
// les parametres necessaires a l`ecriture IGES.
  ...
  // オプションT
  // 楕円，楕円弧要素をパラメトリック・スプライン曲線 (#112) として変換する
  Standard_Integer opt_t = Interface_Static::IVal("write.iges.opt_t");
  // オプションA
  // B-スプライン曲線・曲面は，パラメトリック・スプライン曲線･曲面として出力する。
  Standard_Integer opt_a = Interface_Static::IVal("write.iges.opt_a");
  if(opt_t == 1)
  {
	opt_a = 1;
  }
  if(opt_a == 1)
  {
	  try
	  {
	  Handle(TColStd_HSequenceOfTransient) aryCrv = new TColStd_HSequenceOfTransient();

	  if(IGESConvGeom::ConvertToSplineCurve(mycurve, aryCrv) != 0)
	  {
		  // 変換エラー
		  return res;
	  }
	  ...
	  if(aryCrv->Length() == 1)
	  {
		  // １B-スプライン曲線→１パラメトリック・スプライン曲線
		  res = Handle(IGESData_IGESEntity)::DownCast(aryCrv->Value(1));
	  }
	  else if(aryCrv->Length() > 1)
	  {
		  // １B-スプライン曲線→複数パラメトリック・スプライン曲線
		  Handle(IGESData_HArray1OfIGESEntity) ent3dCrvs = new IGESData_HArray1OfIGESEntity(1, aryCrv->Length());
		  for(Standard_Integer i=1; i<=aryCrv->Length(); i++)
		  {	
			  Handle(IGESData_IGESEntity) c = Handle(IGESData_IGESEntity)::DownCast(aryCrv->Value(i));
			  ent3dCrvs->SetValue(i, c);
		  }
		  Handle(IGESGeom_CompositeCurve) compCrv = new IGESGeom_CompositeCurve();
		  compCrv->Init(ent3dCrvs);
		  res = compCrv;
	  }
	  }
	  catch(Standard_Failure)
	  {
#ifdef _DEBUG
		  cout << "convert to spline curve error!!" << endl;
#endif
	  }
 ...
 const Standard_Real Ufin)
{
  ...
  //if (Abs (Ufin - Udeb - 2 * PI) <= Precision::PConfusion()) {
	...
	Handle(Geom_BSplineCurve) Bspline = ShapeAlgo::AlgoContainer()->ConvertCurveToBSpline(start, Udeb, Ufin, Precision::Approximation(), GeomAbs_C1, 100, 9);
