								    ...
								    const Standard_Real Vfin)
{
  ...
  // –³ŒÀ•½–Ê‚Ì‘Î‰ž
  Handle(IGESGeom_Plane) igsPlan = new IGESGeom_Plane;
  Standard_Real para, parb, parc, pard;
  para = start->Pln().Axis().Direction().X();
  parb = start->Pln().Axis().Direction().Y();
  parc = start->Pln().Axis().Direction().Z();
  pard = para * start->Pln().Location().X() + parb * start->Pln().Location().Y() + parc * start->Pln().Location().Z();

  igsPlan->Init(para, parb, parc, pard, NULL, start->Pln().Location().XYZ(), 0);
  igsPlan->SetFormNumber(0);

  // create Trsf (#124).
  IGESConvGeom_GeomBuilder Build;
  gp_Ax3 Pos;
  Build.SetPosition(Pos);
  Handle(IGESGeom_TransformationMatrix) TMat = Build.MakeTransformation(GetUnit());
  igsPlan->InitTransf(TMat);
