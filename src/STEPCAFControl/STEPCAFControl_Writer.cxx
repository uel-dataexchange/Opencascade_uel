...
#include <STEPConstruct_DataMapOfAsciiStringTransient1.hxx>
...
Standard_Boolean STEPCAFControl_Writer::Transfer( const TDF_LabelSequence1& labels,
						  const STEPControl_StepModelType mode,
						  const Standard_CString multi )
{
  return Transfer( myWriter, labels, mode, multi );
...
Standard_Boolean STEPCAFControl_Writer::Transfer (STEPControl_Writer &writer,
						  const TDF_LabelSequence1 &labels,
						  const STEPControl_StepModelType mode,
						  const Standard_CString multi,
...
Standard_Boolean STEPCAFControl_Writer::WriteExternRefs (const Handle(XSControl_WorkSession) &WS,
							 const TDF_LabelSequence1 &labels) const
{
  if ( labels.Length() <=0 ) return Standard_False;
                                   ...
                                   Standard_Boolean& isDeepest)
{
  ...
  TDF_LabelSequence1 aNextUsageLabs;
                                                                      ...
                                                                      Handle(StepAP242_GeometricItemSpecificUsage)& theGISU)
{
  ...
  const Handle(XSControl_TransferWriter) &TW = WS->TransferWriter();
  const Handle(Transfer_FinderProcess) &FP = TW->FinderProcess();
  //const Handle(Interface_HGraph) aHGraph = WS->Graph();
  //if (aHGraph.IsNull())
  //  return NULL;
  Interface_Graph aGraph = WS->Graph();
...
Handle(StepDimTol_Datum) STEPCAFControl_Writer::WriteDatumAP242(const Handle(XSControl_WorkSession) &WS,
                                                                const TDF_LabelSequence1 theShapeL,
...
static Handle(StepDimTol_HArray1OfDatumSystemOrReference) WriteDatumSystem(const Handle(XSControl_WorkSession) &WS,
                                                                           ...
                                                                           const TDF_LabelSequence1 theDatumSeq,
                                                                           const STEPConstruct_DataMapOfAsciiStringTransient1 theDatumMap,
                                                                           const Handle(StepRepr_RepresentationContext)& theRC)
{
  // Get working data
  const Handle(Interface_InterfaceModel) &Model = WS->Model();
...
void STEPCAFControl_Writer::WriteGeomTolerance (const Handle(XSControl_WorkSession) &WS,
                                                const TDF_LabelSequence1 theShapeSeqL,
                                                const TDF_Label theGeomTolL,
                                                   ...
                                                   const TDF_LabelSequence1  &labels ) const
{
  ...
  TDF_LabelSequence1 DGTLabels;

  STEPConstruct_DataMapOfAsciiStringTransient1 DatumMap;
  ...
  for(i=1; i<=DGTLabels.Length(); i++) {
    ...
    TDF_LabelSequence1 ShapeL;
    TDF_LabelSequence1 aNullSeq;
  ...
  for(i=1; i<=DGTLabels.Length(); i++) {
    ...
    if(kind<20) { //dimension
      ...
      if(kind<35) { // tolerance with datum system
        TDF_LabelSequence1 DatumLabels;
                                                        ...
                                                        const TDF_LabelSequence1  &labels )
{
  ...
  Interface_Graph aGraph = aHGraph->Graph();
  //Handle(XCAFDoc_DimTolTool) DGTTool = XCAFDoc_DocumentTool::DimTolTool(labels(1));
  Handle(XCAFDoc_DimTolTool) DGTTool = XCAFDoc_DocumentTool::DimTolTool(myDoc->Main());
  if(DGTTool.IsNull())
    return Standard_False;
  ...
  TDF_LabelSequence1 aDGTLabels;
  STEPConstruct_DataMapOfAsciiStringTransient1 aDatumMap;
  Handle(StepRepr_RepresentationContext) aRC;
  ...
  for(Standard_Integer i = 1; i <= aDGTLabels.Length(); i++) {
    ...
    TDF_LabelSequence1 aShapeL, aNullSeq;
    DGTTool->GetRefShapeLabel(aDatumL, aShapeL, aNullSeq);
#ifdef _DEBUG
	TDF_Label lblTest = aShapeL.Value(1);
#endif
    Handle(XCAFDoc_Datum) aDatumAttr;
  ...
  for (Standard_Integer i = 1; i <= aDGTLabels.Length(); i++) {
    ...
    TDF_LabelSequence1 aFirstShapeL, aSecondShapeL;
  ...
  for (Standard_Integer i = 1; i <= aDGTLabels.Length(); i++) {
    ...
    TDF_LabelSequence1 aFirstShapeL, aNullSeqL;
    if (!DGTTool->GetRefShapeLabel(aGeomTolL, aFirstShapeL, aNullSeqL))
      continue;
    TDF_LabelSequence1 aDatumSeq;
                                                        ...
                                                        const TDF_LabelSequence1  &labels ) const
{
  ...
  STEPConstruct_DataMapOfAsciiStringTransient1 MapDRI,MapMRI;
  TDF_LabelSequence1 TopLabels;
