...
"input buffer overflow, can't enlarge buffer because scanner uses REJECT" );
		...
		while(_mbsbtype(yy_current_buffer->yy_ch_buf, strlen(yy_current_buffer->yy_ch_buf)-1) == 1){
			// 最後の文字が全角である場合、スキップ
			fseek(yyin, -1, 1);
			memset(yy_current_buffer->yy_ch_buf+strlen(yy_current_buffer->yy_ch_buf)-1, NULL, 1);
		}
		readbuf = &yy_current_buffer->yy_ch_buf[number_to_move];
		while (&readbuf[1] < &yy_current_buffer->yy_ch_buf[yy_n_chars + 1])
		{
			if (iscomment == 0 && readbuf[0] == '/' && readbuf[1] == '*')
			{
				iscomment = 1;
			}
			...
			if (iscomment && readbuf[0] != '\r' && readbuf[0] != '\n')
			{
				readbuf[0] = ' ';
			}
			++readbuf;
...
FILE *file;
	...
	{
		char* interactiveFlg = NULL;
	yy_flush_buffer( b );

	b->yy_input_file = file;
	b->yy_fill_buffer = 1;
	interactiveFlg = getenv("USE_INTERACTIVE");
	if(interactiveFlg != NULL && strcmp("1", interactiveFlg) == 0) {
