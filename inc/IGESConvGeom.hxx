...
//!           all the knots can be passed to C2, all knots which can be are. <br>
  ...
  Standard_EXPORT static Standard_Integer ConvertToSplineSurface(const Handle(Geom_BSplineSurface)& bspSrf, Handle(TColStd_HSequenceOfTransient)& aryParamtricSrf);

  // Convert bsplinecurve to splinecurve
  Standard_EXPORT static Standard_Integer ConvertToSplineCurve(const Handle(Geom_BSplineCurve) &bspCrv, Handle(TColStd_HSequenceOfTransient)& aryParamtricLine);
...
private:
	static Standard_Integer GetKnots(UINT_PTR pageId,const Handle(Geom_BSplineSurface)& bspSrf, Standard_Integer& uknot_num, Standard_Real** uknotsP, Standard_Integer& vknot_num, Standard_Real** vknotsP);
