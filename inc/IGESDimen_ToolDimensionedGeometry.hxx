...
#ifndef _IGESDimen_ToolDimensionedGeometry_HeaderFile
#define _IGESDimen_ToolDimensionedGeometry_HeaderFile
...
//! Tool to work on a DimensionedGeometry. Called by various Modules <br>
...
//!           <IR> detains parameter types and values <br>
  Standard_EXPORT     void ReadOwnParams(const Handle(IGESDimen_DimensionedGeometry)& ent,const Handle(IGESData_IGESReaderData)& IR,IGESData_ParamReader& PR) const;
  //! Writes own parameters to IGESWriter <br>
  Standard_EXPORT     void WriteOwnParams(const Handle(IGESDimen_DimensionedGeometry)& ent,IGESData_IGESWriter& IW) const;
  //! Lists the Entities shared by a DimensionedGeometry <ent>, from <br>
//!           its specific (own) parameters <br>
  Standard_EXPORT     void OwnShared(const Handle(IGESDimen_DimensionedGeometry)& ent,Interface_EntityIterator& iter) const;
  //! Sets automatic unambiguous Correction on a DimensionedGeometry <br>
//!           (NbDimensions forced to 1) <br>
  Standard_EXPORT     Standard_Boolean OwnCorrect(const Handle(IGESDimen_DimensionedGeometry)& ent) const;
  //! Returns specific DirChecker <br>
  Standard_EXPORT     IGESData_DirChecker DirChecker(const Handle(IGESDimen_DimensionedGeometry)& ent) const;
