...
#define NAME_CRV_SIZE	("CURVE_SIZE")	// ê¸ëæÇ≥
#define NAME_CRV_FONT	("CURVE_FONT")	// ê¸éÌ
#define NAME_COLOR_R	("COLOR_RED")
#define NAME_COLOR_G	("COLOR_GREEN")
#define NAME_COLOR_B	("COLOR_BLUE")
...
protected:
  ...
  Standard_EXPORT Standard_Boolean Transfer (STEPControl_Reader& rd, const Standard_Integer num, Handle(TDocStd_Document)& doc, TDF_LabelSequence1& Lseq, const Standard_Boolean asOne = Standard_False);
  ...
  //! Reads define properties assigned to shapes in the STEP
  //! model and assigns them to corresponding labels in the DECAF
  //! document
  Standard_EXPORT Standard_Boolean ReadDefProps(const Handle(XSControl_WorkSession)& WS, Handle(TDocStd_Document)& doc, const STEPCAFControl_DataMapOfPDExternFile& PDFileMap, const XCAFDoc_DataMapOfShapeLabel& ShapeLabelMap) const;
  ...
  Standard_EXPORT void ExpandSubShapes(const Handle(XCAFDoc_ShapeTool)& theShapeTool, const XCAFDoc_DataMapOfShapeLabel& theShapeLabelMap, const STEPCAFControl_DataMapOfShapePD& theShapePDMap, const Handle(TDocStd_Document)& theDoc) const;
  ...
  //! Expands the 3D View
  //! Model STEP entity to Obj.
  Standard_EXPORT void Expand3DView(TDF_Label& theLab, const Handle(StepRepr_RepresentationItem)& theItem, const Handle(TDocStd_Document)& theDoc) const;

  Standard_EXPORT Standard_Boolean ReadView(const Handle(Standard_Transient)& viewEnt, TDF_Label& objLbl, const Handle(XSControl_WorkSession)& theWS, const Handle(TDocStd_Document)& theDoc) const;

  //! Reads Views for instances defined in the STEP model
  Standard_EXPORT Standard_Boolean ReadPMIViews(const Handle(XSControl_WorkSession)& theWS, Handle(TDocStd_Document)& theDoc) const;
